...
class Animal {
    ...
    // Объявляем переменную isMoveCalled для отслеживания выполнения родительской реализации метода move(), которая будет использоваться для задания 8. До выполнения родительской реализации метода значение равно false.
...
class NatureReserve {
    ...
    // Устанавливаем для переменной natureReserve уровень доступа private, для того чтобы данные в нем могли поменять только методами класса NatureReserve
    private var natureReserve: [Animal] = []
